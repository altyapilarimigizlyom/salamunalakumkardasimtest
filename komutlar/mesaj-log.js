...
exports.run = async (client, message, args, params, modlog) => {
  if (!message.member.hasPermission("ADMINISTRATOR"))
    ...
    if(!args[0]) return message.channel.send(new Discord.RichEmbed().setDescription("**Eksik Komut Kullanımı!**").addField("Doğru Kullanım",`\`${a.prefix}mesaj-log ayarla #kanal-ismi\` **veya** \`${a.prefix}mesaj-log sıfırla\``).setColor("RED"));
