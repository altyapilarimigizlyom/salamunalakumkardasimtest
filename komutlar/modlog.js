...
exports.run = async (client, message, args, params, modlog) => {
  if (!message.member.hasPermission("ADMINISTRATOR"))
    ...
    if(!args[0]) return message.channel.send(new Discord.RichEmbed().setDescription("**Eksik Komut Kullanımı!**").addField("__**Doğru kullanım**__",`${a.prefix}modlog ayarla \`#kanaletiketi\`\n**veya**\n${a.prefix}modlog sıfırla`).setColor("#ff3030"));
  if (args[0] === "ayarla") {
    let anan = message.mentions.channels.first();
    await db.set(`genelmodlog_${message.guild.id}`);
